...
test('Attempt to access restricted content using inVALID Cookie Token - custom parameters', async (t) => {
...
test('Attempt to access restricted content with VALID Token but malformed Cookie - custom parameters', async (t) => {
...
test('Access restricted content with VALID Token Cookie - custom parameters', async (t) => {
  const token  = JWT.sign({ id: 123, 'name': 'Charlie' }, secret);
  const options = {
    method: 'POST',
    url: '/privado',
...
test('Access restricted content with VALID Token Cookie (With Options!) - custom parameters', async (t) => {
  const token  = JWT.sign({ id: 123, 'name': 'Charlie' }, secret);
  const options = {
    method: 'POST',
    url: '/privado',
...
test('Authorization Header should take precedence over any cookie - custom parameters', async (t) => {
  const token    = JWT.sign({ id: 123, 'name': 'Charlie' }, secret);
  const options = {
    method: 'POST',
    url: '/privado',
...
test('Valid Google Analytics cookie should be ignored - custom parameters', async (t) => {
...
test('Valid Google Analytics cookie should be ignored (BAD Header Token) - custom parameters', async (t) => {
  const GA = 'gwcm=%7B%22expires%22%3Anull%2C%22clabel%22%3A%22SbNVCILRtFcQwcrE6gM%22%2C%22backoff%22%3A1437241242%7D; _ga=GA1.2.1363734468.1432273334';
...
test('Attempt to access restricted content (with an INVALID URL Token) - custom parameters', async (t) => {
...
test('Try using an incorrect secret to sign the JWT - custom parameters', async (t) => {
...
test('URL Token is well formed but is allowed=false so should be denied - custom parameters', async (t) => {
  // use the token as the 'authorization' header in requests
...
test('Access restricted content (with VALID Token) - custom parameters', async (t) => {
  // use the token as the 'authorization' header in requests
...
test('Attempt to access restricted content using inVALID header tokenType - custom parameters', async (t) => {
...
test('Access restricted content (with VALID Token and header tokenType) - custom parameters', async (t) => {
