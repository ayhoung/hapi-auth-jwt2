...
test('Attempt to access restricted content (with an INVALID URL Token)', async (t) => {
...
test('URL Token is well formed but is allowed=false so should be denied', async (t) => {
  // use the token as the 'authorization' header in requests
...
test('Using route with urlKey=false should be denied', async (t) => {
  // use the token as the 'authorization' header in requests
  let token = JWT.sign({ id: 123, 'name': 'Charlie' }, secret);
  token = '?token=' + token;
  const options = {
...
test('Using route with urlKey=\'\' should be denied', async (t) => {
  // use the token as the 'authorization' header in requests
  let token = JWT.sign({ id: 123, 'name': 'Charlie' }, secret);
  token = '?=' + token;
  const options = {
