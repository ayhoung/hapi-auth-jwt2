...
test('Attempt to access restricted content (without auth token)', async (t) => {
...
test('Attempt to access restricted content (with an INVALID Token)', async (t) => {
...
test('Try using a token with missing characters in body', async (t) => {
  // use the token as the 'authorization' header in requests
  let token = JWT.sign({ id: 123, 'name': 'Charlie' }, secret);
  // delete some characters in body
  const tokenData = token.split('.');
  const header = tokenData[0];
  const body = tokenData[1];
  const signature = tokenData[2];
...
test('Try using an incorrect secret to sign the JWT', async (t) => {
  // use the token as the 'authorization' header in requests
...
test('Try using an expired token', (t) => {
  // use the token as the 'authorization' header in requests
...
test('Token is well formed but is allowed=false so should be denied', async (t) => {
  // use the token as the 'authorization' header in requests
  // const token = jwt.sign({ "id": 1 ,"name":"Old Greg" }, 'incorrectSecret');
...
test('Access restricted content (with VALID Token)', async (t) => {
  // use the token as the 'authorization' header in requests
...
test('Access restricted content (with Well-formed but invalid Token)', async (t) => {
  // use the token as the 'authorization' header in requests
...
test('Request with undefined auth header should 401', async (t) => {
...
test('Auth mode \'required\' should require authentication header', async (t) => {
...
test('Auth mode \'required\' should fail with invalid token', async (t) => {
  // use the token as the 'authorization' header in requests
  const token = JWT.sign({ id: 123, 'name': 'Charlie' }, 'badsecret');
  const options = {
    method: 'POST',
...
test('Auth mode \'required\' should should pass with valid token', async (t) => {
  // use the token as the 'authorization' header in requests
  const token = JWT.sign({ id: 123, 'name': 'Charlie' }, secret);
  const options = {
    method: 'POST',
...
test('Auth mode \'optional\' should pass when no auth header specified', async (t) => {
...
test('Auth mode \'optional\' should fail with invalid token', async (t) => {
  // use the token as the 'authorization' header in requests
  const token = JWT.sign({ id: 123, 'name': 'Charlie' }, 'badsecret');
  const options = {
    method: 'POST',
...
test('Auth mode \'optional\' should pass with valid token', async (t) => {
  // use the token as the 'authorization' header in requests
  // const token = JWT.sign({ id: 123, "name": "Charlie" }, secret);
  const token = JWT.sign({ id: 123, 'name': 'Charlie' }, secret);
  const options = {
...
test('Auth mode \'try\' should pass when no auth header specified', async (t) => {
...
test('Auth mode \'try\' should pass with invalid token', async (t) => {
  // use the token as the 'authorization' header in requests
  const token = JWT.sign({ id: 123, 'name': 'Charlie' }, 'badsecret');
  const options = {
    method: 'POST',
...
test('Auth mode \'try\' should pass with valid token', async (t) => {
  // use the token as the 'authorization' header in requests
  const token = JWT.sign({ id: 123, 'name': 'Charlie' }, secret);
  const options = {
    method: 'POST',
...
test('Scheme should set token in request.auth.token', async (t) => {
  // use the token as the 'authorization' header in requests
  const token = JWT.sign({ id: 123, 'name': 'Charlie' }, secret);
  const options = {
    method: 'GET',
