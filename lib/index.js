...
let assert = require('assert');       // use assert to check if options are set
let JWT = require('jsonwebtoken');    // https://github.com/docdis/learn-json-web-tokens
let extract = require('./extract');   // extract token from Auth Header, URL or Coookie
let pkg = require('../package.json'); // use package name and version rom package.json
let internals = {};                   // see: http://hapijs.com/styleguide#module-globals
...
internals.implementation = function (server, options) {
  ...
  return {
    ...
    authenticate: async function (request, h) {
      ...
      if (typeof options.validateFunc === 'function') {
        ...
        // we map over the key(s) we either return 
        // * h.authenticated for a successful attemp
        // * h.unauthenticated for a failure event when all keys have been tried or 
        //      there is valid key and invalid credentials
        // * false for a filure but with more keys still to try
        const responseArray = keys.map(async function (k) { // itterate through one or more JWT keys
          let verify_decoded;
          try {
          ...
          let credentials;
          try {
            credentials = await options.validateFunc(verify_decoded, request);
          } catch(validate_err) {
          ...
          return h.authenticated({
            credentials: typeof credentials === 'object' ? credentials : decoded,
            artifacts: token
          });
        });
        // since responseArray is an array of promises we need Promise.all to try fulfill them all
        const r = await Promise.all(responseArray);
        // find the first result in the promise that is not false
        return r.find(k => !!k);
      }
      // see: https://github.com/dwyl/hapi-auth-jwt2/issues/130
      let credentials;
      try {
        credentials = await options.verifyFunc(decoded, request);
      } catch(verify_error) {
